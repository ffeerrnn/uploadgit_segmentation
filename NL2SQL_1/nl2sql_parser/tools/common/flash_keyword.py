...
class KeywordProcessor(object):
    ...
    def __setitem__(self, keyword, clean_name=None):
        ...
        if keyword and clean_name:
            ...
            for letter in keyword:
                # print('keyword_trie_dict: ',self.keyword_trie_dict)
                # print('current_dict: ',current_dict)
                # print(self.keyword_trie_dict)
    ...
    def extract_keywords_nlp(self, sentence, span_info=False):
        """Searches in the string for all keywords present in corpus.
        Keywords present are added to a list `keywords_extracted` and returned.
        ...
        while idx < sentence_len:
            ...
            if self._keyword in current_dict or char in current_dict:
                ...
                if char in current_dict:
                    current_dict_continued = current_dict[char]
                    # print (current_dict_continued)

                    idy = idx + 1
                    while idy < sentence_len:
                        inner_char = sentence[idy]
                        # print(inner_char)
                        # print(idy,inner_char)
                        # print(inner_char)
                        ##print(self._keyword)
                        # print(self.non_word_boundaries)
                        # if inner_char not in self.non_word_boundaries and self._keyword in current_dict_continued:
                        if self._keyword in current_dict_continued:
                            # print(inner_char)
                            # update longest sequence found
                            longest_sequence_found = current_dict_continued[self._keyword]
                            sequence_end_pos = idy - 1
                            is_longer_seq_found = True
                        if inner_char in current_dict_continued:
                            current_dict_continued = current_dict_continued[inner_char]
                            # print(current_dict_continued)
                        ...
                        if self._keyword in current_dict_continued:
                            ...
                            sequence_end_pos = idy - 1
                            # print (sentence[sequence_end_pos])
                            is_longer_seq_found = True
                    if is_longer_seq_found:
                        idx = sequence_end_pos
                        # print('qazwsx',idx)
            ...
            if idx + 1 >= sentence_len:
                if self._keyword in current_dict:
                    sequence_found = current_dict[self._keyword]
                    keywords_extracted.append((sequence_found, sequence_start_pos, sentence_len+1))
            idx += 1
            if reset_current_dict:
                reset_current_dict = False
                sequence_start_pos = idx
...
if __name__ == '__main__':
    ...
    # with open('name_search2.txt', 'r') as f1:
    #     for line1 in f1:
    #         keyword_processor.add_keyword(line1.replace('\n', '').strip())
    keyword_processor.add_keyword('张又侠')
    # keyword_processor.__getitem__()
    # keyword_processor.add_keyword('ab人')
    keywords_found = keyword_processor.extract_keywords_nlp(r'许其亮张又侠和蔡奇参加首都义务植树活动-新华网许其亮', span_info=True)
    # keywords_found = key
    # word_processor.extract_keywords('abc def')
